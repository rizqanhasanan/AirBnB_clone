...
def do_show(self, args):
    ...
    if len(args) == 0:
        print("** class name missing **")
        return                                                           if len(args) == 1:                                                       print("** instance id missing **")                                   return            
    storage = FileStorage()
    storage.reload()                                                     object_dict = storage.all()
    ...
    try:
       eval(args[0])
        ...
        return
    keys = args[0] + "." + args[1]
    keys = args[0] + "." + args[1]                                       try:                                                                    Value = object_dict[keys]                                            print(Value)        
    except KeyError:
        ...
        def do_count(self, args):
            ...
            storage = FileStorage()
            storage.reload()
            objects = storage.all()                                              try:                                                                    if len(args) != 0:                                                       eval(args)                
               except NameError:
                   print("** class doesn't exist **"
                   return                                                           for keys, value in objects.items():                                      if len(args) != 0:                                                       if type(value) is eval(args):                                            object_list.append(value)                                     else:
