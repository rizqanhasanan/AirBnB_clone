...
def save(self):
    ...
    with open(type(self).__file_path, 'w+') as f:                            for key, value in dic.items():                                           if isinstance(value, BaseModel):
                value2 = value.to_dict()
                type(self).__objects[key] = value2
            if isinstance(value, User):
                value2 = value.to_dict()                                             type(self).__objects[key] = value2                               if isinstance(value, State):                                             value2 = value.to_dict() 
                type(self).__objects[key] = value2
            if isinstance(value, City):                                              value2 = value.to_dict()
                type(self).__objects[key] = value2
            if isinstance(value, Amenity):
                value2 = value.to_dict()
                type(self).__objects[key] = value2                               if isinstance(value, Place):                                             value2 = value.to_dict()                                             type(self).__objects[key] = value2
            if isinstance(value, Review):
                value2 = value.to_dict()    
                type(self).__objects[key] = value2                          json.dump(type(self).__objects, f)
    ...
    if isfile(type(self).__file_path):
        with open(type(self).__file_path, 'r') as f:
            ...
            if dic2['__class__'] == 'User':
                instance = User(**dic2)
            if dic2['__class__'] == 'State':                                         instance = State(**dic2)                                         if dic2['__class__'] == 'City':
                instance = City(**dic2)
            if dic2['__class__'] == 'Amenity':
                instance = Amenity(**dic2)
            if dic2['__class__'] == 'Place':                                         instance = Place(**dic2)                                         if dic2['__class__'] == 'Review':
